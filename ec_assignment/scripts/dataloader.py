...
class Triplet_dataset_with_mine_EC(torch.utils.data.Dataset):
    ...
    def __init__(self, id_ec, ec_id, mine_neg, args):
        self.id_ec = id_ec
        self.ec_id = ec_id
        self.full_list = []
    ...
    def __getitem__(self, index):
        ...
        a = torch.load(f'./data/{self.fingerprint}/' + anchor + '.pt')
        p = torch.load(f'./data/{self.fingerprint}/' + pos + '.pt')
        n = torch.load(f'./data/{self.fingerprint}/' + neg + '.pt')
        return format_esm(a), format_esm(p), format_esm(n)


class MultiPosNeg_dataset_with_mine_EC(torch.utils.data.Dataset):

    def __init__(self, id_ec, ec_id, mine_neg, n_pos, n_neg, args):
        self.id_ec = id_ec
        self.ec_id = ec_id
        self.n_pos = n_pos
        self.n_neg = n_neg
    ...
    def __getitem__(self, index):
        anchor_ec = self.full_list[index]
        anchor = random.choice(self.ec_id[anchor_ec])
        a = format_esm(torch.load(f'./data/{self.fingerprint}/' +
                       anchor + '.pt')).unsqueeze(0)
        data = [a]
        for _ in range(self.n_pos):
            pos = random_positive(anchor, self.id_ec, self.ec_id)
            p = format_esm(torch.load(f'./data/{self.fingerprint}/' +
                           pos + '.pt')).unsqueeze(0)
            data.append(p)
        for _ in range(self.n_neg):
            neg = mine_negative(anchor, self.id_ec, self.ec_id, self.mine_neg)
            n = format_esm(torch.load(f'./data/{self.fingerprint}/' +
                           neg + '.pt')).unsqueeze(0)
            data.append(n)
