...
def main(opt):
    ...
    prediction_lines = pd.read_csv(opt.predictions+f"_mode{opt.mode}.csv",sep='\t')
    for index, row in tqdm(prediction_lines.iterrows()):
        for i in range(opt.beam_size):
            pred_smi = row[f"sampled_smiles_{i+1}"]
            predictions.append(pred_smi)

    data_size = len(predictions) // (opt.augmentation * opt.beam_size) if opt.length == -1 else opt.length
    lines = predictions[:data_size * (opt.augmentation * opt.beam_size)]
    ...
    results_iter = pool.imap(func=canonicalize_smiles_clear_map, iterable=lines)

    # 用tqdm包装迭代器
    raw_predictions = list(tqdm(results_iter, total=len(lines), desc="Processing SMILES"))
    # raw_predictions = pool.map(func=canonicalize_smiles_clear_map, iterable=lines)
    pool.close()
    pool.join()

    predictions = [[[] for j in range(opt.augmentation)] for i in
                   range(data_size)]  # data_len x augmentation x beam_size
    for i, line in enumerate(raw_predictions):
        predictions[i // (opt.beam_size * opt.augmentation)][
            i % (opt.beam_size * opt.augmentation) // opt.beam_size].append(line)
    ...
    with open(opt.targets, 'r') as f:
        ...
        results_iter = pool.imap(func=canonicalize_smiles_clear_map, iterable=targets)
        # 用tqdm包装迭代器
        targets = list(tqdm(results_iter, total=len(targets), desc="Processing SMILES"))
    ...
    if opt.detailed:
        ...
        df.to_csv(f"/home/tiantao/bioretro/bioformer/outputs/detailed_results_{opt.mode}.csv")
...
if __name__ == "__main__":
    ...
    parser.add_argument('-mode', type=str, default="0",
                       help='finetunemode')
    parser.add_argument('-predictions', type=str, default="outputs/predicted_reactants")
