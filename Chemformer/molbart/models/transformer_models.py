...
class BARTModel(_AbsTransformerModel):
    ...
    def forward(self, x, get_attn=False):
        ...
        if get_attn:
            ...
            # memory = self.encoder(encoder_embs, src_key_padding_mask=encoder_pad_mask, get_attn)
            memory = encoder_embs
            all_attn = []

            for layer in self.encoder.layers:  # self.encoder.layers 是 nn.ModuleList
                memory, attn = layer(memory, src_key_padding_mask=encoder_pad_mask, get_attn=True)
            ...
            for layer in self.decoder.layers:  # self.layers 是 ModuleList
            ...
            return encoder_input, output, all_attn, self_attn_list, cross_attn_list
