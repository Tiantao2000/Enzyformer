...
def main(args):
    ...
    for idx in tqdm(range(len(pred_reactants_list))):
        ...
        reac = output_tokens[1:-1]
        prod = input_tokens[1:-1]
        print(f"reac is {reac}")
        print(f"prod is {prod}")
        print("a")
        indices = np.argwhere(layer_attn_0).tolist()
        for pp in indices:
            a,b = pp[0],pp[1]
            print(f"key for reac is {reac[a]}, all reac is {''.join(reac)},the number is {a}")
            print(f"key for prod is {prod[b]}, all prod is {''.join(prod)},the number is {b}")
        print("a")
